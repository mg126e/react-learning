...
import {
  addTodo as oldAddTodo,
  createTodo,
  removeTodo as oldRemoveTodo,
  toggleTodo as oldToggleTodo,
} from "../utils/todoHelpers.ts";
...
export function useTodos() {
  ...
  const addTodo = (name: string) =>
    setTodos((prevTodos) => {
      localStorage.setItem(
        "todos",
        JSON.stringify(oldAddTodo(prevTodos, createTodo(name))),
      );
  ...
  const removeTodo = (id: string) =>
    setTodos((prevTodos) => {
      localStorage.setItem(
        "todos",
        JSON.stringify(oldRemoveTodo(prevTodos, id)),
      );
  ...
  const toggleTodo = (id: string) =>
    setTodos((prevTodos) => {
      localStorage.setItem(
        "todos",
        JSON.stringify(oldToggleTodo(prevTodos, id)),
      );
